---
title: GIS for Designers
subtitle: A course on Geographic Information Systems for landscape architects, urban planners, and other designers.
description: Tutorials on terrain modeling, hydrology, geomorphometry, landscape ecology. and urban analytics in GIS.
featured_image: /images/
---

![New Orleans](/images/new-orleans-render-2.png)

## Syllabus
A guide to Geographic Information Systems
for landscape architects, urban planners, and other designers.
Topics include terrain modeling, hydrology, landscape ecology, and urban analytics.
Watch tutorials on
[<i class="fab fa-youtube"></i> Youtube](https://www.youtube.com/channel/UCmGEF6Bf1SO92oLQoGCPDTw/)
or [<i class="fab fa-vimeo-v"></i> Vimeo](https://vimeo.com/baharmon).
Read the [latest syllabus](https://github.com/baharmon/syllabi/blob/master/gis-for-designers-syllabus-2019.pdf?raw=true)
or [archived syllabi](https://github.com/baharmon/syllabi).

**Software:**
[GRASS GIS](https://grass.osgeo.org/),
[QGIS](https://qgis.org/),
[ArcGIS](https://www.esri.com/), &
[Rhino](https://www.rhino3d.com/)

## Datasets

* [Natural Earth Dataset for GRASS GIS](https://zenodo.org/record/3762852/files/natural-earth-dataset.zip?download=1)
* [Governor's Island Dataset for GRASS GIS](https://zenodo.org/record/3940780/files/nyspf_govenors_island.zip?download=1)
* [List of Geospatial Data Sources](geospatial-data-sources)

---

## Tutorials

* [<i class="fab fa-vimeo-v"></i>](...)[<i class="fab fa-youtube"></i>](...)[ An Introduction to GRASS GIS](intro-to-grass)
* [<i class="fab fa-vimeo-v"></i>](...)[<i class="fab fa-youtube"></i>](...)[ Acquiring Global Data for GRASS GIS](global-data-for-grass)
* [<i class="fab fa-vimeo-v"></i>](...)[<i class="fab fa-youtube"></i>](...)[ Acquiring Urban Data for GRASS GIS](urban-data-for-grass)
* [<i class="fab fa-vimeo-v"></i>](...)[<i class="fab fa-youtube"></i>](...)[ Terrain Analysis in GRASS GIS](terrain-analysis-in-grass)
